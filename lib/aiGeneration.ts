...
2) documents.marketResearch — 시장조사 초안(마크다운). 아이디어/요구사항 유형(앱·서비스 / 콘텐츠·SNS / B2B·SaaS / 커머스 등)에 맞는 조사 관점만 선별해 아래 스켈레톤 범위 안에서 작성합니다.
   - 정보가 부족해도 각 섹션에 "조사 필요" 같은 placeholder를 넣지 말고, 아이디어를 바탕으로 합리적인 가설·초안을 작성합니다.
   - 외부 검증이 꼭 필요한 항목만 마지막 "## 추가 조사 필요 항목"에 한 번만 모읍니다(섹션마다 반복 금지).
   - 랜딩 페이지·최소 거래 페이지·고객 수락 체인·첫 고객 확보·콘텐츠 검증·검증 순서 등 GTM/검증 항목은 시장조사가 아니므로 제외합니다(제품화 전략에서 다룸).
${MARKET_RESEARCH_SKELETON}
