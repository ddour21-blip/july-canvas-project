...
async function hit(key: string, max: number, windowMs: number): Promise<boolean> {
  ...
  return getAdminDb().runTransaction(async (tx) => {
    ...
    if (now - d.windowStart >= windowMs) {
      // 윈도우 만료 → 리셋.
      tx.update(ref, { count: 1, windowStart: now, expiresAt: now + windowMs, updatedAt: FieldValue.serverTimestamp() });
      return true;
    }
    if (d.count >= max) return false; // 초과 — 증가하지 않음.
    tx.update(ref, { count: d.count + 1, expiresAt: d.windowStart + windowMs, updatedAt: FieldValue.serverTimestamp() });
    return true;
  });
}
